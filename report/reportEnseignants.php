...
header("Content-disposition: filename=report_Enseignants.csv"); 
...
<?php 
	...
	$year=date('Y');
		...
		foreach ($data as $d)
			{ 
	 	$sql1 = 'SELECT (SELECT a.NOM FROM etablissements a WHERE a.ETABLISSEMENT_ID = '.$d->ETABLISSEMENT_ID.') AS LibelleETABLISSEMENT FROM enseignants c';
	 	$stmt = $dbh->query($sql1);
		$stmt->execute();
		$result1=$stmt->fetch();
