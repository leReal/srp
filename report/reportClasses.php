...
<?php 
	...
	$year=date('Y');
		...
		foreach ($data as $d)
			{ 
	 	$sql1 = 'SELECT (SELECT a.NOM FROM etablissements a WHERE a.ETABLISSEMENT_ID = '.$d->ETABLISSEMENT_ID.') AS LibelleETABLISSEMENT FROM classes c';
	 	$stmt = $dbh->query($sql1);
		$stmt->execute();
		$result1=$stmt->fetch();
		
				$datas[] = array(
				'ID Classes' => $d->CLASSE_ID,
                'Etablissement'=> $result1->LibelleETABLISSEMENT,
				'Nom de la classe' => $d->NOM,
				'Niveau' => $d->NIVEAU,
				'Frais de la scolarite' => $d->FRAIS_SCOLARITE,
				'Frais inscription' => $d->FRAIS_INCRIPTION,
			  );
			}
			$i=0;
			foreach($datas as $v){
